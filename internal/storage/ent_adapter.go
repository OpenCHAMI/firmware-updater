...
func FromEntResource(ctx context.Context, entResource *ent.Resource) (interface{}, error) {
	...
	case "FirmwareBundle":
		...
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for FirmwareBundle: %w", err)
		}
		...
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for FirmwareBundle: %w", err)
			}
		}
	...
	case "FirmwareUpdateJob":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for FirmwareUpdateJob: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for FirmwareUpdateJob: %w", err)
			}
		}
