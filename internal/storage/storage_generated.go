...
// LoadAllFirmwareBundles loads all FirmwareBundle resources from Ent storage
func LoadAllFirmwareBundles(ctx context.Context) ([]*v1.FirmwareBundle, error) {
	if entClient == nil {
	...
	entResources, err := entClient.Resource.Query().
		Where(entresource.KindEQ("FirmwareBundle")).
		WithLabels().
		WithAnnotations().
		All(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to load FirmwareBundle resources: %w", err)
	}

	// Convert to Fabrica resources
	var resources []*v1.FirmwareBundle
	for _, entResource := range entResources {
		...
		resources = append(resources, fabricaResource.(*v1.FirmwareBundle))
	}

	return resources, nil
}

// LoadFirmwareBundle loads a single FirmwareBundle resource by UID from Ent storage
func LoadFirmwareBundle(ctx context.Context, uid string) (*v1.FirmwareBundle, error) {
	if entClient == nil {
		return nil, fmt.Errorf("ent client not initialized")
	}
	...
	entResource, err := entClient.Resource.Query().
		Where(
			...
			entresource.KindEQ("FirmwareBundle"),
		).
		WithLabels().
		WithAnnotations().
		Only(ctx)
	...
	return fabricaResource.(*v1.FirmwareBundle), nil
}

// SaveFirmwareBundle saves a FirmwareBundle resource to Ent storage
func SaveFirmwareBundle(ctx context.Context, resource *v1.FirmwareBundle) error {
	if entClient == nil {
	...
	if err != nil {
		return fmt.Errorf("failed to convert FirmwareBundle to ent: %w", err)
	}
	...
	if err != nil && !ent.IsNotFound(err) {
		return fmt.Errorf("failed to check FirmwareBundle existence: %w", err)
	}
...
// DeleteFirmwareBundle deletes a FirmwareBundle resource from Ent storage
func DeleteFirmwareBundle(ctx context.Context, uid string) error {
	if entClient == nil {
		return fmt.Errorf("ent client not initialized")
	}
	...
	if err != nil {
		return fmt.Errorf("failed to delete FirmwareBundle %s: %w", uid, err)
	}
...
func (b *entStorageBackend) LoadAll(ctx context.Context, resourceType string) ([]json.RawMessage, error) {
	...
	switch resourceType {
	case "FirmwareBundle":
		resources, err := LoadAllFirmwareBundles(ctx)
		if err != nil {
			return nil, err
		}
...
func (b *entStorageBackend) Load(ctx context.Context, resourceType, uid string) (json.RawMessage, error) {
	...
	switch resourceType {
	case "FirmwareBundle":
		resource, err := LoadFirmwareBundle(ctx, uid)
		if err != nil {
			return nil, err
		}
...
func (b *entStorageBackend) Save(ctx context.Context, resourceType, uid string, data json.RawMessage) error {
	...
	case "FirmwareBundle":
		...
		if err := json.Unmarshal(data, &res); err != nil {
			return fmt.Errorf("failed to unmarshal FirmwareBundle: %w", err)
		}
		res.Metadata.UID = uid
		return SaveFirmwareBundle(ctx, &res)
	case "FirmwareUpdateJob":
...
func (b *entStorageBackend) Delete(ctx context.Context, resourceType, uid string) error {
	...
	switch resourceType {
	case "FirmwareBundle":
		return DeleteFirmwareBundle(ctx, uid)
	case "FirmwareUpdateJob":
...
func (c *StorageClient) Get(ctx context.Context, kind, uid string) (interface{}, error) {
	...
	switch kind {
	case "FirmwareBundle":
		var resource v1.FirmwareBundle
		if err := json.Unmarshal(raw, &resource); err != nil {
			return nil, fmt.Errorf("failed to unmarshal FirmwareBundle: %w", err)
		}
		return &resource, nil
	case "FirmwareUpdateJob":
...
func (c *StorageClient) Update(ctx context.Context, resource interface{}) error {
	...
	case *v1.FirmwareBundle:
		return c.backend.Save(ctx, "FirmwareBundle", res.Metadata.UID, data)
