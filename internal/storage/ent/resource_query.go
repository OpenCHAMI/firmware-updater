...
import (
	"context"
	"database/sql/driver"
	"firmware-manager/internal/storage/ent/annotation"
	"firmware-manager/internal/storage/ent/label"
	"firmware-manager/internal/storage/ent/predicate"
	"firmware-manager/internal/storage/ent/resource"
	"fmt"
	"math"
...
func (_q *ResourceQuery) QueryLabels() *LabelQuery {
...
func (_q *ResourceQuery) QueryAnnotations() *AnnotationQuery {
...
func (_q *ResourceQuery) WithLabels(opts ...func(*LabelQuery)) *ResourceQuery {
	query := (&LabelClient{config: _q.config}).Query()
...
func (_q *ResourceQuery) WithAnnotations(opts ...func(*AnnotationQuery)) *ResourceQuery {
	query := (&AnnotationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resource, error) {
	...
	if query := _q.withLabels; query != nil {
		if err := _q.loadLabels(ctx, query, nodes,
			func(n *Resource) { n.Edges.Labels = []*Label{} },
			func(n *Resource, e *Label) { n.Edges.Labels = append(n.Edges.Labels, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAnnotations; query != nil {
		if err := _q.loadAnnotations(ctx, query, nodes,
			func(n *Resource) { n.Edges.Annotations = []*Annotation{} },
			func(n *Resource, e *Annotation) { n.Edges.Annotations = append(n.Edges.Annotations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ResourceQuery) loadLabels(ctx context.Context, query *LabelQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Label)) error {
	...
	query.Where(predicate.Label(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(resource.LabelsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.resource_labels
		if fk == nil {
			return fmt.Errorf(`foreign-key "resource_labels" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "resource_labels" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ResourceQuery) loadAnnotations(ctx context.Context, query *AnnotationQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Annotation)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Resource)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Annotation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(resource.AnnotationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.resource_annotations
		if fk == nil {
			return fmt.Errorf(`foreign-key "resource_annotations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "resource_annotations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
