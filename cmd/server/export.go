...
Examples:
  ...
  firmware_manager export --kinds FirmwareBundle --output ./firmwarebundle-backup
...
func runExport(ctx context.Context, format, output string, kinds []string, perType bool) error {
	...
	if len(kinds) > 0 {
		...
		resourceKinds = []string{"FirmwareBundle", "FirmwareUpdateJob"}
	}
...
func exportResourceKind(ctx context.Context, kind, output, format string, perType bool) (int, error) {
	...
	switch kind {
	case "FirmwareBundle":
		items, e := storage.Queryfirmwarebundles(ctx).All(ctx)
		if e != nil {
			return 0, fmt.Errorf("failed to query firmwarebundles: %w", e)
		}
	...
	case "FirmwareUpdateJob":
		...
		if e != nil {
			return 0, fmt.Errorf("failed to query firmwareupdatejobs: %w", e)
		}
		for _, item := range items {
			resources = append(resources, item)
		}
