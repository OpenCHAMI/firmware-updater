...
// This file provides the generated boilerplate for FirmwareBundle reconciler.
//
...
//
// To customize reconciliation logic, edit firmwarebundle_reconciler.go
package reconcilers
...
// FirmwareBundleReconciler reconciles FirmwareBundle resources.
//
// This reconciler:
//   - Observes FirmwareBundle resources and updates their Status
...
//
// The implementation of reconcileFirmwareBundle() is in firmwarebundle_reconciler.go
type FirmwareBundleReconciler struct {
	reconcile.BaseReconciler

	// Custom fields are defined in firmwarebundle_reconciler.go
}

// NewDefaultFirmwareBundleReconciler creates a default FirmwareBundle reconciler.
//
...
//
// Returns:
//   - *FirmwareBundleReconciler: Initialized reconciler
func NewDefaultFirmwareBundleReconciler(client reconcile.ClientInterface, eventBus events.EventBus) *FirmwareBundleReconciler {
	return &FirmwareBundleReconciler{
...
func (r *FirmwareBundleReconciler) GetResourceKind() string {
	return "FirmwareBundle"
}

// Reconcile brings FirmwareBundle to desired state.
//
// This method is called:
//   - When a FirmwareBundle resource is created/updated/deleted
...
//   - resource: The FirmwareBundle resource to reconcile
//
// Returns:
//   - Result: Indicates if/when to requeue
//   - error: If reconciliation failed
func (r *FirmwareBundleReconciler) Reconcile(ctx context.Context, resource interface{}) (reconcile.Result, error) {
	...
	var res v1.FirmwareBundle // This is the typed struct
	...
	r.Logger.Debugf("Reconciling FirmwareBundle %s/%s", res.Kind, res.GetUID())

	// Call custom reconciliation logic (now passing &res)
	if err := r.reconcileFirmwareBundle(ctx, &res); err != nil {
		r.Logger.Errorf("Reconciliation failed for FirmwareBundle %s: %v", res.GetUID(), err)
	...
	if err := r.UpdateStatus(ctx, &res); err != nil {
		r.Logger.Errorf("Failed to update status for FirmwareBundle %s: %v", res.GetUID(), err)
	...
	/*
		...
		eventType := "io.openchami.inventory.firmwarebundles.reconciled"
		if err := r.EmitEvent(ctx, &res, eventType); err != nil {
			r.Logger.Warnf("Failed to emit event for FirmwareBundle %s: %v", res.GetUID(), err)
